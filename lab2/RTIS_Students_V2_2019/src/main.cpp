	...
	std::vector<PointLightSource>* &lightSourceList)
{
	...
	Shape * infinitePlane = new InfinitePlane(Vector3D(0, -1, 0), Vector3D(0, 1, 0), green_50);

	// Store the objects in the object list
	objectsList->push_back(s1);
	objectsList->push_back(s2);
	objectsList->push_back(s3);
	objectsList->push_back(infinitePlane);
	...
	PointLightSource p1(Vector3D(1, 5, 0), Vector3D(10, 10, 10));
	lightSourceList->push_back(p1);

	PointLightSource p2(Vector3D(2, 1, 0), Vector3D(20, 20, 20));
	lightSourceList->push_back(p2);
	...
	std::vector<Shape*>* &objectsList, std::vector<PointLightSource>* &lightSourceList)
{
	...
	Vector3D intensity = Vector3D(10); // Radiant intensity (watts/sr)
...
int main()
{
	...
	Shader *shader = new GlobalShader(3, bgColor, Vector3D(0.3));
